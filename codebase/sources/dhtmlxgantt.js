				...
				if (gantt.config.calendar == 'persian' && persianDate) {
					if (unit == "year") {
						...
						while (+curr < +end) {
							...
							curr = (new persianDate(curr)).add('years', 1).toDate();
						}
		...
		var dateHelper = {
			...
			date_to_str: function (format, utc) {
				...
				if (gantt.config.calendar == 'persian' && persianDate) {
					...
					body = "date = new persianDate(date); return \"" + format + "\";";
				}
				// end
				else {
					format = format.replace(/%[a-zA-Z]/g, function (a) {
				...
				if (utc) body = body.replace(/date\.get/g, "date.getUTC");
				var dateToStr = new Function("date", "to_fixed", "locale", "getISOWeek", body);
